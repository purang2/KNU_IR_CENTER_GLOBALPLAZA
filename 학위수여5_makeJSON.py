...
#        print(sql)
    elif schoolname != "경영학전공" and schoolname != "회계학전공":
...
def create_Json_grade_Top10(n):
    ...
    for year in range(n, n+1):
        ...
        for i in range(0, len(school_name_list)):
            ...
            temp_dictonary = {
                              "연도": str(year),
                              "전공": school_name_list[i],
                              "계열": get_major_types(school_name_list[i]),
                              "학사_남": int(male_bech),
                              "학사_여": int(female_bech),
                              "석사_남": int(male_master),
                              "석사_여": int(female_master),
                              "박사_남": int(male_phd),
                              "박사_여": int(female_phd)}
            json_list.append(temp_dictonary)

    with open('학위수여_5_전공별성비_'+str(n)+'.json', 'w', encoding='UTF-8') as make_file:
        json.dump(json_list, make_file, indent="\t", ensure_ascii=False)

for i in range(2016, 2021):
    create_Json_grade_Top10(i)
